class Dialogue extends Phaser.GameObjects.Container {
    ...
    constructor( scene, x, y, color, bool ){
        ...
        this.espace = this.scene.add.text( x1 / proportionZoom, y + this.dialTextUp.height*2, 'ESPACE pour continuer', { fontFamily: "BalbeerRustic", fontSize: 100* proportionZoom + 'px', fontStyle: "bold", color: WhiteText } ).setVisible(false).setOrigin(0);
    ...
    update(){
        ...
        if( keys.SPACE.isDown && this.switchText && this.ready ){
            ...
            if( this.transReady ){
                if( this.scene.level == 0 && this.indexCurDial == 4  ){
            ...
            this.spawnDial();
            if( this.scene.level == 0 && this.indexCurDial == 5  ){
                // Apparition des touches
                this.scene.activeTouche( "D" );
                this.scene.activeTouche( "Q" );
            }
        }
